...
Inbound applications support both confidential (server-side) and public (SPA/mobile) clients, customizable scopes for user attributes, permissions, and external connections, and per-app session settings.
...
resource "descope_inbound_application" "backend_api" {
  ...
  approved_callback_urls = ["https://api.example.com/callback"]
}
```
