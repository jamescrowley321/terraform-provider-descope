...
func TestManagementKeyCRUD(t *testing.T) {
	...
	// Create
	h.LoadFixture("management_key/create.tf")
	out := h.Apply(nameVar)
	assert.Contains(t, out, "Apply complete!")

	attrs := h.StateResource("descope_management_key.test")
	...
	h.LoadFixture("management_key/update.tf")
	out = h.Apply(nameVar)
	assert.Contains(t, out, "Apply complete!")
	...
	h.StateRM("descope_management_key.test")
	h.LoadFixture("management_key/create.tf")
	h.Import("descope_management_key.test", id, nameVar)
